...
func (b *InternalBackend) Call(
	...
	case "import_skill_registry_git":
		...
		return result, nil
	case "publish_skill_registry":
		return b.callSkillRegistryHandler(ctx, args, handlePublishSkillRegistry)
	case "audit_skill_registry":
		return b.callSkillRegistryHandler(ctx, args, handleAuditSkillRegistry)
	case "get_skill_registry":
		return b.callSkillRegistryHandler(ctx, args, handleGetSkillRegistryWithRegistry)
	case "delete_skill_registry":
		if b.skillReg == nil {
			return errorResult("skill registry not initialised"), nil
		}
		return b.callSkillRegistryHandler(ctx, args, handleDeleteSkillRegistryWithRegistry)
...
func (b *InternalBackend) callSkillRegistryHandler(
	ctx context.Context, args json.RawMessage,
	build func(*skillregistry.Registry) handlerFunc,
) (json.RawMessage, error) {
	if b.skillReg == nil {
		return errorResult("skill registry not initialised"), nil
	}
	result, err := build(b.skillReg)(ctx, b.store, args)
	if err != nil {
		return errorResult(err.Error()), nil
	}
	return result, nil
