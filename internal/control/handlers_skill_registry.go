...
func handleGetSkillRegistry(
	...
	if p.Version > 0 {
		// Try global first, then any workspace.
		entry, err = s.GetSkillRegistryEntry(ctx, nil, p.Name, p.Version)
	} else {
		entry, err = s.GetSkillRegistryHead(ctx, store.SkillScope{IncludeAll: true}, p.Name)
	}
	if err != nil {
...
func handleDeleteSkillRegistry(
	ctx context.Context, s store.Store, args json.RawMessage,
) (json.RawMessage, error) {
	var p struct {
		Name    string `json:"name"`
	...
	// Admin delete: try every scope until one succeeds. workspaceID nil = global.
	delErr := s.SoftDeleteSkillRegistryEntry(ctx, nil, p.Name, p.Version)
	if delErr != nil {
