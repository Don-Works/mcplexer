...
func TestSPAFallback_SWVersionBumped(t *testing.T) {
	...
	if !strings.Contains(string(sw), "mcplexer-shell-v13") {
		t.Error("sw.js CACHE_NAME should use v13")
	}
}
...
func TestSPAFallback_RedirectsNonLoopbackHTTPToPublicURL(t *testing.T) {
	...
	h := spaFallback(fsys, http.FileServerFS(fsys), "", "https://max-mac.example.ts.net")

	req := httptest.NewRequest(http.MethodGet, "http://max-mac:13333/app?source=pwa", nil)
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
	...
	if got, want := rec.Header().Get("Location"), "https://max-mac.example.ts.net/app?source=pwa"; got != want {
