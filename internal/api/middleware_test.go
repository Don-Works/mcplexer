...
func TestSecurityHeadersMiddlewareCOOPTrustworthyOrigins(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			target: "https://peer.example:13333/api/v1/health",
			want:   "same-origin",
		},
		{
			name:    "forwarded https gets coop",
			target:  "http://peer.example:13333/api/v1/health",
...
func TestBrowserOriginProtectionMiddleware(t *testing.T) {
	...
	t.Run("allows trusted-host origin", func(t *testing.T) {
		trusted := browserOriginProtectionMiddleware([]string{"peer.example", "  HOST.lan.  "})(noop)
		req := httptest.NewRequest(http.MethodGet, "http://peer.example:13333/api/v1/workspaces", nil)
		req.Header.Set("Origin", "http://peer.example:13333")
		rr := httptest.NewRecorder()
	...
	t.Run("trusted-host list still blocks others", func(t *testing.T) {
		trusted := browserOriginProtectionMiddleware([]string{"peer.example"})(noop)
		req := httptest.NewRequest(http.MethodGet, "http://peer.example:13333/api/v1/workspaces", nil)
		req.Header.Set("Origin", "https://evil.example")
		rr := httptest.NewRecorder()
		trusted.ServeHTTP(rr, req)
...
func TestCORSMiddleware(t *testing.T) {
	...
	t.Run("trusted host preflight allowed", func(t *testing.T) {
		trusted := corsMiddleware([]string{"peer.example"})(noop)
		req := httptest.NewRequest(http.MethodOptions, "http://peer.example:13333/api/v1/workspaces", nil)
		req.Header.Set("Origin", "http://peer.example:13333")
		rr := httptest.NewRecorder()
		trusted.ServeHTTP(rr, req)
		if rr.Code != http.StatusNoContent {
		...
		if got := rr.Header().Get("Access-Control-Allow-Origin"); got != "http://peer.example:13333" {
			t.Fatalf("unexpected allow-origin header: %q", got)
		}
	})
