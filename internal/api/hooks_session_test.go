...
func TestSessionHookEndInjectsCaptureNudge(t *testing.T) {
	...
	resp := decodeSessionResp(t, rr)
	if resp.HookSpecificOutput == nil {
		t.Fatal("expected hookSpecificOutput on SessionEnd")
	}
	ctx := resp.HookSpecificOutput.AdditionalContext
	if !strings.Contains(ctx, "CAPTURE BEFORE ENDING") {
		t.Errorf("SessionEnd should carry the capture nudge, got %q", ctx)
	}
	if !strings.Contains(ctx, "memory.save") {
		t.Errorf("SessionEnd should name memory.save, got %q", ctx)
	}
}
...
func TestSessionHookStopDoesNotFloodCaptureNudge(t *testing.T) {
	...
	// SessionEnd still emits the capture nudge exactly once.
	...
	if resp.HookSpecificOutput == nil ||
		!strings.Contains(resp.HookSpecificOutput.AdditionalContext, "CAPTURE BEFORE ENDING") {
		t.Fatalf("SessionEnd must still emit the capture nudge, got %+v", resp.HookSpecificOutput)
	}
}
