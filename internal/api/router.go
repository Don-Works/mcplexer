...
func NewRouter(deps RouterDeps) http.Handler {
	...
	if deps.ApprovalManager != nil {
		...
		mux.Handle("/v1/hooks/pretool", loopbackHookMiddleware(http.HandlerFunc(hooks.pretool)))
		...
		mux.Handle("/v1/hooks/session", loopbackHookMiddleware(http.HandlerFunc(hooks.session)))
	}
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if _, err := fs.Stat(staticFS, p); err == nil {
			if isHTMLPath(p) {
				...
				if apiToken != "" {
					setSessionCookie(w, apiToken, sessionCookieSecure(r, publicBase))
				}
			} else if p == "sw.js" {
...
// sessionCookieSecure keeps local HTTP development working while ensuring the
// production browser origin cannot receive a cleartext session cookie. A
// canonical HTTPS public URL acts as an HTTPS requirement for non-loopback
// hosts; direct TLS and TLS-terminating proxies are detected from the request.
func sessionCookieSecure(r *http.Request, publicBase *url.URL) bool {
	if r == nil {
		return publicBase != nil && strings.EqualFold(publicBase.Scheme, "https")
	}
