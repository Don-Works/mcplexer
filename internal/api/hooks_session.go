...
//
// SessionEnd is a cleanup/notification event in Claude Code's hook
// contract: it accepts ONLY the universal output fields (systemMessage,
// continue, ...) and rejects hookSpecificOutput/additionalContext, which
// belong to SessionStart. Returning additionalContext here made Claude
// Code fail hook-output validation and exit non-zero, breaking every
// claude_cli session (and delegated worker) at teardown. The nudge is
// delivered via systemMessage instead — the only channel SessionEnd has.
func sessionEndResponse() SessionHookResponse {
	return SessionHookResponse{
		SystemMessage: sessionEndNudge,
	}
}
