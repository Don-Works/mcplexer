...
func TestUsersListHidesSyntheticDeviceIdentities(t *testing.T) {
	...
	st.peers["peer-legacy"] = store.P2PPeer{PeerID: "peer-legacy", DisplayName: "max-air"}
...
func TestUsersListHidesLegacyDeviceNamedIdentities(t *testing.T) {
	...
	st := newFakeUserStore()
	st.users["u-self"] = store.User{UserID: "u-self", DisplayName: "Max", IsSelf: true}
	st.users["u-device-name"] = store.User{UserID: "u-device-name", DisplayName: "max-air"}
	...
	st.peers["peer-device"] = store.P2PPeer{PeerID: "peer-device", DisplayName: "max-air"}
