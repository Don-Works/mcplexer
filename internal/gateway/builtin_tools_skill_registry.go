...
func skillRegistryToolDefinitions() []Tool {
	return []Tool{
		...
		},
		{
			...
			Description: "Fetch one SKILL.md from the registry. Explicit, content-pinned skill includes are expanded in body by default; raw_body, content_hash, expanded_sha256, and provenance make the render auditable. Set expand_includes=false to return the raw stored body. Pass an optional version (integer, latest, or stable). When include_bundle=true, bundle_b64 carries any attached tar.gz.",
			InputSchema: json.RawMessage(`{
				"type": "object",
				"properties": {
					"name": {
					...
					"expand_includes": {
						"type": "boolean",
						"description": "Expand exact content-pinned include markers in body. Defaults to true. Set false to return only the raw stored body."
					}
				},
				"required": ["name"]
			}`),
