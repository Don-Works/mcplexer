...
const maxAuditToolErrorBytes = 2000

// extractToolErrorText joins the text content from an isError result. Code
// mode emits lint guidance before the runtime/downstream error, so taking only
// the first block made audit rows describe a warning while hiding the cause.
func extractToolErrorText(result json.RawMessage) string {
	...
	if len(joined) > maxAuditToolErrorBytes {
		cut := maxAuditToolErrorBytes
