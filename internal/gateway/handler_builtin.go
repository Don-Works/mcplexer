...
func (h *handler) handleBuiltinCall(
	...
	switch req.Name {
	case "mcpx__workspace_read_file", "mcpx__workspace_list_directory",
		"mcpx__workspace_write_file", "mcpx__workspace_edit_file":
		return h.handleWorkspaceTool(ctx, req)
	case "mcpx__search_tools":
		args, err := decodeSearchToolsArgs(req.Arguments)
		if err != nil {
			return nil, &RPCError{Code: CodeInvalidParams, Message: err.Error()}
		}
		return h.handleDiscoverTools(
	...
	case "mcpx__delegate_batch":
		return h.handleDelegateBatch(ctx, req.Arguments)
