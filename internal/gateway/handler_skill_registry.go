...
func (h *handler) resolvePublishScope(ctx context.Context, scope string) (*string, error) {
	...
	case "auto":
		...
		return &ws, nil
	default:
		return nil, fmt.Errorf("invalid scope %q: expected \"auto\", \"workspace\", or \"global\"", scope)
	}
}
...
func (h *handler) handleSkillGet(ctx context.Context, raw json.RawMessage) (json.RawMessage, *RPCError) {
	...
	if args.IncludeBundle && entry.BundleSHA256 != "" {
		bundle, sha, fetchErr := h.skillRegistry.FetchBundle(ctx, h.sessionScope(ctx), args.Name, ref)
...
// skillGetResponse is the structured mcpx__skill_get payload. Body holds
// the verbatim SKILL.md markdown (frontmatter + instructions).
