	...
	return []Tool{
		{
			...
			InputSchema: json.RawMessage(`{
				...
				"properties": {
					...
					"repo_brief": {"type":"string","description":"Optional pre-computed repository brief (architecture, key paths, conventions) injected into the worker prompt so the worker skips cold-start re-exploration. Counted against the worker input-token budget; empty or whitespace is ignored."},
					"no_auto_context": {"type":"boolean","description":"Opt out of automatic context injection. By default the worker prompt is enriched with recalled memory and recent mesh activity for the workspace (bounded, best-effort); set true to suppress it."},
					...
					"task_kind": {"type":"string","description":"Optional task category used for model routing/review, e.g. coding, review, architecture, tool_calling, visual. When omitted it is inferred from the task shape so kind-aware ranking still engages."},
					"task_shape": {"type":"string","enum":["review","research","codebase_scan","small_edit","multi_file"],"description":"Optional structural shape of the work (scale/topology). Auto-classified from objective+handoff when omitted; worker_mode=review forces review."},
					...
					"worker_isolation": {"type":"string","enum":["worktree","none"],"description":"Filesystem mode. Defaults to worktree: a unique git worktree outside the parent checkout, retained through hooks/output/persistence then removed. none is an explicit trusted escape, including the required mode for CLI-backed delegates."},
					"touches_files": {"type":"array","items":{"type":"string"},"description":"Optional concrete workspace-relative write paths. Paths are canonicalized at runtime inside the isolated worktree; traversal, globs, dynamic expansion, and symlink escapes are rejected."},
					...
					"baseline_tokens_estimate": {"type":"integer","description":"Caller-supplied estimate of incremental frontier-model tokens this delegated slice would have consumed. This is the delegating model's claim, not a measured value. Authoritative measured metrics are real_dollars_spent, frontier_quota_preserved, and frontier_quota_burned reported per delegation. Also used as a break-even signal: a single execute delegation whose estimate is below ~8000 tokens returns a non-blocking warning that setup overhead likely exceeds the savings — do such small slices inline unless you need isolation, a review, or parallel fan-out."},
					...
					"progress_interval": {"type":"integer","description":"Emit a low-priority delegation_progress mesh finding every N worker tool calls so the parent sees mid-run status. Default 10; pass -1 to disable. API providers only — CLI adapters run as one opaque send."},
					...
					"verify_citations": {"type":"boolean","description":"Re-derive every file:line citation in the worker's output from the code index and REJECT the run (status=blocked) if a citation is contradicted. Deterministic, model-free; needs the read-only index tools (in the default allowlist). Best for weak/local models that drift on line numbers. Injects a vetted post_execute_script: rejected if you also supply your own post_execute_script, or if tool_allowlist_json/capability_preset scopes out the index tools."},
					"capability_preset": {"type":"string","enum":["full","coder","researcher","minimal"],"description":"Sizes the delegate's allowed tool surface + mcplexer features to its trust. Omit for today's behavior (full default surface gated only by tool_allowlist_json). full = unrestricted; coder = discovery/invocation wrappers + selected downstream + task/memory write, no mesh/secret/subdelegation; researcher = read-only (no writes, no subdelegation); minimal = bare slim surface (discovery/invocation wrappers only, no downstream/builtins). Composes with tool_allowlist_json: both must pass, so a profile can only narrow."},
					"capability_profile": {
						"type":"object",
		...
		{
			Name:        "mcpx__delegate_batch",
			Description: "Delegate several independent units of work in one call. Each item is a full delegate_worker payload (objective, handoff, model, caps, touches_files, ...). Every item is validated up front (fail fast before any worker is created), cross-item touches_files overlaps are surfaced as batch warnings, then each item is delegated through the normal path so coordination, context injection, and shape routing behave identically to a single delegation. Items are independent: a mid-batch failure is reported in that item's error field and does NOT roll back earlier successes. Use this instead of many delegate_worker calls when you have a known set of parallel sub-tasks; share common repository context once via shared_repo_brief. Max 20 items and 60 total dispatched workers per batch. For capacity-mode ranking across many items, prefer explicit model candidates — each capacity item re-scans recent delegations.",
			InputSchema: json.RawMessage(`{
				"type": "object",
				"properties": {
					"delegations": {
						"type":"array",
						"description":"1..20 delegate_worker payloads. Each accepts the same fields as mcpx__delegate_worker (objective is required per item).",
						"items": {"type":"object","properties":{"objective":{"type":"string"}},"required":["objective"]}
					},
					"shared_repo_brief": {"type":"string","description":"Optional repository brief prepended to every item's repo_brief so common context is stated once."}
				},
			...
			Extras: withExamples(ToolAnnotations{
				...
				`{"delegations":[{"objective":"summarise internal/gateway/ changes","model_provider":"opencode_cli","model_id":"minimax/MiniMax-M3"},{"objective":"summarise internal/store/ changes","model_provider":"grok_cli","model_id":"grok-build"}],"shared_repo_brief":"MCPlexer: Go MCP gateway. Focus on the last 5 commits."}`,
			}),
		},
		{
		...
		},
		{
			...
			InputSchema: json.RawMessage(`{
				...
				"properties": {
					...
					"worker_isolation": {"type":"string","enum":["worktree","none"],"description":"Defaults to worktree. none is an explicit trusted escape, including the required mode for CLI-backed delegates."},
					"touches_files": {"type":"array","items":{"type":"string"},"description":"Concrete workspace-relative paths the isolated worker may write."},
		...
		},
		{
			...
			Description: "Block until a previously dispatched delegation reaches a terminal state (success, partial, failure, blocked, cancelled, interrupted, or needs_review), or until the timeout elapses. Returns compact aggregate worker counts, tokens, cost, tool calls, and timeout status.",
			InputSchema: json.RawMessage(`{
				"type": "object",
				"properties": {
