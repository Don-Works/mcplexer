...
If an `mcplexer` MCP server is configured here, `search_tools` discovers capabilities, `call_tool` invokes one small independent target, and `execute_code` handles composed workflows; those wrappers reach the `memory`, `task`, and `mesh` namespaces.
