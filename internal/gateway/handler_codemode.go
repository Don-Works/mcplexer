...
func (h *handler) handleCodeExecute(
	...
	// Reuse the wrapper call's ID so outer and inner audit rows can be joined.
	// Direct/internal callers that bypass handleToolsCall still get one here.
...
func (h *handler) gatherCodeModeTools(ctx context.Context) ([]Tool, error) {
	if _, isolated := workerFilesystemScopeFromContext(ctx); isolated {
		// Exact worktree mode never introspects arbitrary downstreams. Start from
		// gateway-owned definitions so a same-name external namespace cannot
		// shadow schemas or trigger catalog auto-start before the contract filter.
		allTools := append([]Tool(nil), h.codeModeBuiltinTools()...)
		if h.skillRegistry != nil {
			allTools = append(allTools, skillRegistryToolDefinitions()...)
		}
		allTools = append(allTools, workspaceToolDefinitions()...)
		allTools = dedupeToolsByName(allTools)
		allTools = filterByWorkerFilesystemContract(ctx, allTools)
		allTools = h.applyToolHints(ctx, allTools)
		return allTools, nil
	}
	...
	filtered = filterByWorkerFilesystemContract(ctx, filtered)
...
func (h *handler) codeModeToolDefs(ctx context.Context) ([]codemode.ToolDef, error) {
	...
	tools = filterByWorkerToolAllowlist(ctx, tools)
	tools = filterByWorkerCapability(ctx, tools)
	tools = filterByWorkerFilesystemContract(ctx, tools)
