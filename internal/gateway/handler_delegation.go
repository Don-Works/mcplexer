...
// applyDelegationSessionDefaults fills workspace + parent context from the
// session and injects disabled providers, then authorizes the workspace
// write. Shared by delegate_worker and delegate_batch so both hops apply
// identical defaulting and access control. Returns an RPCError to reject.
func (h *handler) applyDelegationSessionDefaults(ctx context.Context, in *workersadmin.DelegationInput) *RPCError {
	if in.WorkspaceID == "" {
		in.WorkspaceID = h.currentWorkspaceID(ctx)
	}
	if in.WorkspaceID == "" {
		return &RPCError{Code: CodeInvalidParams, Message: "workspace_id required (session is not bound to a workspace)"}
	}
...
func (h *handler) handleDelegateWorker(ctx context.Context, raw json.RawMessage) (json.RawMessage, *RPCError) {
	...
	if rpc := h.applyDelegationSessionDefaults(ctx, &in); rpc != nil {
		return nil, rpc
	}
...
func (h *handler) handleDelegateBatch(ctx context.Context, raw json.RawMessage) (json.RawMessage, *RPCError) {
	if h.workerAdmin == nil {
		return marshalErrorResult("worker delegation is not enabled"), nil
	}
	var in workersadmin.BatchDelegationInput
	if err := json.Unmarshal(raw, &in); err != nil {
		return nil, &RPCError{Code: CodeInvalidParams, Message: err.Error()}
	}
	if len(in.Delegations) == 0 {
		return nil, &RPCError{Code: CodeInvalidParams, Message: "delegations required"}
	}
	// Every item is defaulted + write-authorized against the session
	// before any worker is created; a single unauthorized workspace
	// rejects the whole batch.
	for i := range in.Delegations {
		if rpc := h.applyDelegationSessionDefaults(ctx, &in.Delegations[i]); rpc != nil {
			return nil, rpc
		}
	}
	out, err := h.workerAdmin.DelegateBatch(ctx, in)
	if err != nil {
		return marshalErrorResult(err.Error()), nil
	}
	return marshalJSONResult(out)
