...
func (h *handler) handleToolsCall(
	...
	// Worktree isolation uses an exact gateway-owned contract. Reject an
	// unknown name before route lookup, schema discovery, fuzzy recovery, or
	// any downstream auto-start work; argument-specific checks run later at
	// the concrete dispatch boundary.
	if err := guardWorkerFilesystemToolName(ctx, req.Name); err != nil {
	...
	if routeErr := guardWorkerFilesystemRoute(ctx, req.Name, routeResult.DownstreamServerID); routeErr != nil {
		rpcErr := &RPCError{Code: CodeInvalidParams, Message: routeErr.Error()}
	...
	// Worktree-isolated workers are guarded at the concrete inner-call
	// boundary, after JavaScript evaluation and fuzzy-name recovery. This is
	// intentionally runtime enforcement, not lexical scanning of execute_code.
	if workerOrigin {
		guarded, guardErr := guardWorkerFilesystemArgs(ctx, req.Name, req.Arguments)
		if guardErr != nil {
			rpcErr := &RPCError{Code: CodeInvalidParams, Message: guardErr.Error()}
			h.recordAuditBlocked(ctx, req.Name, req.Arguments, routeResult, nil, rpcErr, start)
			return nil, rpcErr
		}
		req.Arguments = guarded
	}
