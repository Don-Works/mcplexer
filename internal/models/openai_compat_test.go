...
func TestOpenAICompatForwardsThinkingMode(t *testing.T) {
	body := `{
		"choices":[{"message":{"role":"assistant","content":"ok"},"finish_reason":"stop"}],
		"usage":{"prompt_tokens":1,"completion_tokens":1}
	}`
	...
	adapter, err := NewAdapter(Config{
		Provider: ProviderOpenAICompat, ModelID: "glm-5.2",
		EndpointURL: "https://api.z.ai/api/paas/v4", HTTPClient: client,
		Thinking: "disabled",
	})
	if err != nil {
		t.Fatalf("NewAdapter: %v", err)
	}
	if _, err := adapter.Send(context.Background(), SendRequest{
		Messages: []Message{{Role: RoleUser, Content: "triage"}},
	}); err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestOpenAICompatRejectsInvalidThinkingMode(t *testing.T) {
	_, err := NewAdapter(Config{
		Provider: ProviderOpenAICompat, ModelID: "glm-5.2",
		EndpointURL: "https://example.com/v1", Thinking: "sometimes",
	})
	if err == nil || !strings.Contains(err.Error(), "thinking must be enabled or disabled") {
		t.Fatalf("err = %v", err)
	}
}
