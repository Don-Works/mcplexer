...
// Tool execution is still sandboxed via claudeCLISandboxConfig (read-only
// Claude auth/config and no access to ~/.mcplexer), so prompt-injection cannot
...
func claudeExecRunner(ctx context.Context, binary string, args []string, stdin string, workspacePath string) ([]byte, []byte, error) {
	return runSandboxedModelCLI(
		...
		claudeCLISandboxConfig, claudeCLIEnvironmentPolicy(),
	)
}
