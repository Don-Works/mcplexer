...
func piExecRunner(ctx context.Context, binary string, args []string, prompt string, workspacePath string) ([]byte, []byte, error) {
	wrapper := sandbox.NewCommandWrapper(piCLISandboxConfig())
	program, wrappedArgs, cleanup := wrapper.Wrap(binary, args)
	defer cleanup()

	cmd := newSandboxedCLICmd(ctx, program, wrappedArgs)
	// SECURITY (L7): feed the composed prompt via stdin, not argv, so the
	// system + user text never appears in `ps`. Pi consumes piped stdin
	// (non-TTY) as the initial message in non-interactive mode.
	if prompt != "" {
...
func piCLISandboxConfig() sandbox.Config {
