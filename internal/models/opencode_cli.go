...
// runs it. The sandbox denies WRITES to ~/.claude and ~/.mcplexer so a
// prompt-injection that talks the model into installing a malicious
// hook or rewriting our gateway DB is blocked at the kernel layer,
// while reads stay open so OAuth flows + provider creds still work.
func opencodeExecRunner(ctx context.Context, binary string, args []string, stdin string, workspacePath string) ([]byte, []byte, error) {
	wrapper := sandbox.NewCommandWrapper(opencodeCLISandboxConfig())
	...
	// hard-stop terminates the real `opencode` process tree — not just
	// the sandbox-exec wrapper — and reclaims the token budget promptly.
...
// opencodeCLISandboxConfig returns the sandbox config for every
// opencode subprocess. Network is host-passthrough until the
// mcplexer-proxy egress filter lands. Mirrors claudeCLISandboxConfig
// — same dotfile denies plus an extra DenyWritePaths entry for
// ~/.mcplexer (gateway DB + secrets) to stop a sub-agent from rewriting
// our own config.
func opencodeCLISandboxConfig() sandbox.Config {
