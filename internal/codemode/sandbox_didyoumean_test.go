...
func TestSandbox_PunctuatedToolMemberAliasDispatchesCanonicalName(t *testing.T) {
	caller := newMockCaller()
	tools := []ToolDef{{
	...
	result, err := sandbox.Execute(
		context.Background(), `print(gcal.get_current_time({}));`, tools,
	)
	if err != nil {
		t.Fatal(err)
	}
	if result.Error != "" {
		t.Fatalf("Code Mode alias failed: %s", result.Error)
	}
	caller.mu.Lock()
	defer caller.mu.Unlock()
	if len(caller.calls) != 1 || caller.calls[0].Name != "gcal__get-current-time" {
		t.Fatalf("alias dispatched %+v, want canonical gcal__get-current-time", caller.calls)
	}
}
