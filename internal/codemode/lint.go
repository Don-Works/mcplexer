...
var (
	...
	// Fallback for syntactically incomplete snippets. Valid code uses the AST
	// binding collector below so parameters, loops, catches, and destructuring
	// are also understood.
...
func lintToolCallTypos(code string, lines []string, toolNames []string) []LintWarning {
	...
	localBindings := collectLocalBindings(code)
	var out []LintWarning
	...
	for _, m := range matches {
		...
		if knownMembers, ok := namespaces[ns]; ok {
			...
			} else if sug := crossNamespaceToolSuggestions(ns, member, namespaces); len(sug) > 0 {
		...
		if sug := DidYouMean(ns, nsList, 1); len(sug) > 0 {
			...
			out = append(out, LintWarning{
				Line:     line,
				Message:  fmt.Sprintf("%s.%s is not a registered tool. Did you mean: %s?", ns, member, strings.Join(sug, ", ")),
				Severity: "warning",
			})
		}
	}
...
func indexToolNames(toolNames []string) (map[string][]string, map[string]struct{}) {
	...
	members := make(map[string]struct{}, len(toolNames)*2)
	for ns, names := range namespaces {
		// Sort to match groupByNamespace so collision-suffix aliases agree
		// with what typegen/sandbox actually register.
		sort.Strings(names)
		aliases := assignJSMemberNames(names)
		seen := make(map[string]bool, len(names)*2)
		indexed := make([]string, 0, len(names)*2)
		for _, name := range names {
			for _, member := range []string{name, aliases[name]} {
				if member == "" || seen[member] {
					continue
				}
				seen[member] = true
				indexed = append(indexed, member)
				members[ns+"__"+member] = struct{}{}
			}
		}
		namespaces[ns] = indexed
	}
...
func joinDotted(ns string, members []string) string {
	...
	for _, member := range members {
		call := renderJSCallName(ns + "__" + member)
		if !seen[call] {
			seen[call] = true
...
func crossNamespaceToolSuggestions(ns, member string, namespaces map[string][]string) []string {
	targets := map[string]bool{member: true, ns + "_" + member: true}
	var suggestions []string
	seen := make(map[string]bool)
	for otherNS, knownMembers := range namespaces {
		for _, known := range knownMembers {
			if !targets[known] && !strings.HasSuffix(known, "_"+member) {
				continue
			}
			call := renderJSCallName(otherNS + "__" + known)
			if !seen[call] {
				seen[call] = true
				suggestions = append(suggestions, call)
			}
		}
	}
