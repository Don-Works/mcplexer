...
func (s *Sandbox) Execute(ctx context.Context, code string, tools []ToolDef) (*ExecutionResult, error) {
	...
	s.toolNames = make([]string, 0, len(tools))
	for ns, entries := range groups {
		for _, entry := range entries {
			s.toolNames = append(s.toolNames, ns+"__"+entry.name)
			if entry.jsName != entry.name {
				s.toolNames = append(s.toolNames, ns+"__"+entry.jsName)
			}
		}
	}
...
func renderJSCallName(toolName string) string {
	if ns, member, ok := strings.Cut(toolName, "__"); ok {
		return ns + "." + jsMemberAlias(member)
	}
	return "_global." + jsMemberAlias(toolName)
}
