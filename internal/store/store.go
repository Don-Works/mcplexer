...
// WorkspaceIDs lists visible workspace IDs from most-specific to least-
// specific, alongside global rows (nil-workspace). An empty slice means
// "global only". Pass IncludeAll=true to bypass scoping (admin operations +
// import wizards), where no workspace hierarchy is available.
...
type SkillRegistryStore interface {
	...
	// ListSkillRegistryScopeHeads returns the latest active row for every
	// (workspace, name) pair in scope. Unlike ListSkillRegistryHeads it does
	// not collapse shadowed rows, making it suitable for admin inventory and
	// consistency audits.
	ListSkillRegistryScopeHeads(ctx context.Context, scope SkillScope, limit int) ([]SkillRegistryEntry, error)
