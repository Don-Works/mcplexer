...
type WorkerRun struct {
	...
	// ResultBranch/ResultCommit/ResultChanged are the trusted runner's
	// authoritative git snapshot for worktree-isolated delegations. A clean
	// snapshot still records its base branch/commit but ResultChanged remains
	// false, so readers must not present the base HEAD as a deliverable.
...
type WorkerRunFinalize struct {
	...
	ResultCommit       string
	ResultChanged      bool
	// HasGitResult distinguishes an authoritative clean snapshot
	// (ResultChanged=false) from an unrelated terminal-status update that must
	// preserve any existing recovery/artifact fields.
	HasGitResult bool
}
