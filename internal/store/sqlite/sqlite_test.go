...
// TestJournalModeIsWAL locks in that the DSN pragmas actually apply: the
// historical mattn-style params were silently ignored by modernc, leaving
// the store in rollback-journal mode where readers block writers — fatal
// once reads run on a concurrent pool.
func TestJournalModeIsWAL(t *testing.T) {
	db := newTestDB(t)
	var mode string
	if err := db.Raw().QueryRowContext(context.Background(), "PRAGMA journal_mode").Scan(&mode); err != nil {
...
// TestReadsDoNotQueueBehindWriteTx: with the writer pinned at
// MaxOpenConns(1) by an open transaction, reads must be served from the
// concurrent read pool instead of queueing behind the writer — the
// queueing is what made every dashboard/tool read inherit write latency.
func TestReadsDoNotQueueBehindWriteTx(t *testing.T) {
	db := newTestDB(t)
	ctx := context.Background()

	err := db.Tx(ctx, func(store.Store) error {
		rctx, cancel := context.WithTimeout(ctx, 3*time.Second)
		defer cancel()
		if _, err := db.ListWorkspaces(rctx); err != nil {
			return fmt.Errorf("read while a write tx holds the writer connection: %w", err)
		}
		return nil
	})
	if err != nil {
