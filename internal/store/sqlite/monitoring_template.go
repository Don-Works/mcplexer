...
const logTemplateCols = `id, source_id, masked, severity, count,
    ...
    acked, ack_note, triaged_at, triaged_severity`
...
func (d *DB) UpsertLogTemplate(ctx context.Context, t *store.LogTemplate, n int64) (bool, error) {
	...
	err := d.withTx(ctx, func(q queryable) error {
		var triagedSeverity string
		err := q.QueryRowContext(ctx,
			`SELECT triaged_severity FROM log_templates WHERE id = ?`, t.ID,
		).Scan(&triagedSeverity)
		switch {
		case errors.Is(err, sql.ErrNoRows):
			...
			return nil
		case err != nil:
			return fmt.Errorf("read log template triage state: %w", err)
		}

		// Refresh severity on every occurrence. A real severity increase
		// re-opens durable triage; same/lower repeats stay classified and are
		// rolled into the mapped incident occurrence below without AI spend.
		resetTriage := severityHigher(t.Severity, triagedSeverity)
		_, err = q.ExecContext(ctx, `
			UPDATE log_templates SET count = count + ?, last_seen = ?,
				sample_last = ?, severity = ?,
				triaged_at = CASE WHEN ? THEN NULL ELSE triaged_at END
			WHERE id = ?`,
			n, formatTime(t.LastSeen.UTC()), t.SampleLast, t.Severity,
			resetTriage, t.ID)
		if err != nil {
			return fmt.Errorf("upsert log template: %w", err)
		}
		return recordMappedMonitoringOccurrence(ctx, q, t, n)
	})
	return isNew, err
}
...
// ListPendingLogTemplates is the durable queue behind the log-watch worker.
// Rows stay here until commit_triage completes; a rolling window or digest
// budget therefore cannot permanently hide a lower-ranked template.
func (d *DB) ListPendingLogTemplates(ctx context.Context, sourceIDs []string, limit int) ([]*store.LogTemplate, error) {
	if len(sourceIDs) == 0 {
		return []*store.LogTemplate{}, nil
	}
	query := `SELECT ` + logTemplateCols + ` FROM log_templates
		WHERE source_id IN (` + placeholders(len(sourceIDs)) + `)
		  AND acked = 0 AND triaged_at IS NULL
		ORDER BY CASE severity
			WHEN 'critical' THEN 3 WHEN 'error' THEN 2 WHEN 'warn' THEN 1 ELSE 0 END DESC,
			last_seen DESC, count DESC`
	...
	rows, err := d.q.QueryContext(ctx, query, args...)
	if err != nil {
		return nil, fmt.Errorf("list pending log templates: %w", err)
	}
	defer func() { _ = rows.Close() }()
	out := []*store.LogTemplate{}
	for rows.Next() {
		t, err := scanLogTemplate(rows)
		if err != nil {
			return nil, fmt.Errorf("scan pending log template: %w", err)
		}
		out = append(out, t)
	}
	return out, rows.Err()
...
func (d *DB) AckLogTemplate(ctx context.Context, id, note string) error {
	res, err := d.q.ExecContext(ctx,
		`UPDATE log_templates SET acked = 1, ack_note = ?,
		 triaged_at = ?, triaged_severity = severity WHERE id = ?`,
		note, formatTime(time.Now().UTC()), id)
	if err != nil {
...
func scanLogTemplate(row interface{ Scan(...any) error }) (*store.LogTemplate, error) {
	...
	err := row.Scan(&t.ID, &t.SourceID, &t.Masked, &t.Severity, &t.Count,
		...
		&acked, &t.AckNote, &triagedAt, &t.TriagedSeverity)
	if err != nil {
		return nil, err
	}
	t.Acked = acked != 0
	...
	if triagedAt.Valid && triagedAt.String != "" {
		at := parseTime(triagedAt.String)
		t.TriagedAt = &at
	}
	return &t, nil
}
