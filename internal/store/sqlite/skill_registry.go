...
func (d *DB) GetSkillRegistryHead(
	...
	order, orderParams := skillRegistryScopeOrder(scope)
	q := `
		SELECT ` + skillRegSelectCols + `
		FROM skill_registry_entries
		WHERE name = ? AND deleted_at IS NULL ` + clause + `
		ORDER BY ` + order + `, version DESC, workspace_id ASC
		LIMIT 1`
	args := append([]any{name}, params...)
	args = append(args, orderParams...)
...
func (d *DB) ListSkillRegistryHeads(
	...
	q := `
		...
		WHERE rn = 1
		ORDER BY name ASC`
	args := append(orderParams, params...)
	if limit > 0 {
		q += ` LIMIT ?`
...
// skillRegistryScopeOrder preserves the caller's workspace precedence.
// Session scopes are ordered most-specific first, so a child workspace must
// beat a parent even when the parent has a numerically newer skill version.
func skillRegistryScopeOrder(scope store.SkillScope) (string, []any) {
	if scope.IncludeAll {
		return "(workspace_id IS NULL) ASC", nil
	}
	if len(scope.WorkspaceIDs) == 0 {
		return "(workspace_id IS NULL) DESC", nil
	}
	parts := make([]string, 0, len(scope.WorkspaceIDs))
	params := make([]any, 0, len(scope.WorkspaceIDs))
	...
	for _, id := range scope.WorkspaceIDs {
		...
		seen[id] = struct{}{}
		parts = append(parts, "WHEN ? THEN "+strconv.Itoa(len(parts)))
		params = append(params, id)
	}
	return "CASE workspace_id " + strings.Join(parts, " ") +
		" ELSE " + strconv.Itoa(len(parts)) + " END", params
}
