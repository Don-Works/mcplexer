...
// irreducible always-allow bypass — the two-tool worker surface. Gating the
...
func (p *CapabilityProfile) Allows(toolName string, isWriteClass bool) (bool, string) {
	...
	// The two-tool worker surface is the irreducible entrypoint: it bypasses
	// the namespace and read-only gates so search/execute always work, but
