...
//   - no workspace lookup wired (nil) → deny scoped delivery. Missing ACL
//     wiring must never widen access; empty/global broadcasts are handled by
...
func (t *MeshTransport) peerAuthorizedForWorkspace(ctx context.Context, peerID, workspaceID string) bool {
	...
	if t.wsLookup == nil {
		t.logger.Warn("p2p mesh: workspace binding lookup is not configured; dropping broadcast leg",
			"peer", peerID, "workspace", workspaceID)
		return false
	}
