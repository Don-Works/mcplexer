...
// lookup is permissive (used only in tests / slim wiring).
...
//
// host, source, sink must be non-nil. lookup + wsLookup may be nil for
// tests (permissive). auditor + logger may be nil.
...
// the set of workspaces the peer is paired with. A nil wsLookup is
// permissive (tests). A lookup error or an unbound peer yields an empty
// slice (default-deny) so a delta never leaks cross-workspace agents.
