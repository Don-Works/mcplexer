...
// TestDHTEnablesIPDiversityFilter guards the Sybil-resistance option on every
// DHT constructed by MCPlexer. A configured-but-empty filter returns a
// non-nil stats slice; an unconfigured routing table returns nil.
func TestDHTEnablesIPDiversityFilter(t *testing.T) {
	h := startDHTTestHost(t, "diversity-filter")
	defer func() { _ = h.Close() }()

	if stats := h.dht.GetRoutingTableDiversityStats(); stats == nil {
		t.Fatal("DHT routing table has no IP diversity filter")
	}
}
