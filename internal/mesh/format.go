...
func FormatSendResult(msg *store.MeshMessage) string {
	return fmt.Sprintf("Sent %s [%s] — expires %s | id: %s",
		msg.Kind, msg.Priority, formatUntil(msg.ExpiresAt), msg.ID)
}
...
// formatUntil renders a future timestamp as a compact countdown.
// Counterpart to formatRelativeTime, which is past-oriented and renders
// any future time as "just now".
