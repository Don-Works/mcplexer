...
### Five top-level tools — everything else is code mode
...
- ` + "`mcpx__retrieve`" + ` — expand a ` + "`[[ccr key=...]]`" + ` compression marker back to its exact original content.
...
- ` + "`index.*`" + ` — local code index. Start with ` + "`index.context({query, budget_tokens})`" + ` for a ranked pack with source snippets; use ` + "`index.search({query, limit})`" + ` for citation-ready implementation/behavior source, ` + "`index.symbols`" + ` for definitions, ` + "`index.deps`" + ` for blast radius, and ` + "`index.map_failure`" + ` for a pasted failure. Lexical source search is always available; semantic search is explicit and local-only.
...
- ` + "`mcpx.skill_search`" + ` / ` + "`mcpx.skill_get`" + ` — versioned, mesh-shared skill registry. Search it before building a capability from scratch; prefer registry skills over local ` + "`~/.claude/skills/`" + ` copies. The separate ` + "`skill.*`" + ` namespace is run telemetry only, not registry lookup.
