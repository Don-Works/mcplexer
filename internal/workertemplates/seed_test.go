...
func TestLogWatchPromptIsCanonical(t *testing.T) {
	...
	for _, want := range []string{
		`monitoring.commit_triage`,
		`pending_only:true`,
		`Entries omitted by the token budget remain durably pending`,
		`daemon—not you—deduplicates incident classes`,
		`never call tool search or help`,
		`Never use session or kv`,
		`const d = monitoring.digest({window:"15m", budget_tokens:200, max_samples:1, pending_only:true}); print(typeof d === "string" ? d : d.text);`,
		`Do not call the digest again`,
		`merely reading, reasoning, or returning blank is not completion`,
	} {
		if !strings.Contains(tmpl.PromptTemplate, want) {
			t.Fatalf("canonical log-watch prompt missing durable triage rule %q", want)
		}
	}
	if strings.Contains(tmpl.PromptTemplate, `task.list({`) || strings.Contains(tmpl.PromptTemplate, `monitoring.notify({`) {
		t.Fatal("canonical log-watch prompt must use the single commit operation")
	}
	if strings.Contains(tmpl.PromptTemplate, `budget_tokens:3000`) ||
		strings.Contains(tmpl.PromptTemplate, `budget_tokens:1800`) ||
		strings.Contains(tmpl.PromptTemplate, `budget_tokens:900`) {
		t.Fatal("canonical log-watch prompt must keep the digest below execute-code output caps")
	}
}
