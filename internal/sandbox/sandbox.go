...
type Config struct {
	...
	//
	// On the darwin sandbox-exec driver this renders as a
	// `(deny file-write-data (literal "..."))` rule WITHOUT the
	// file-read-data clause. On bwrap/Linux this currently maps to a
	// best-effort approximation — see the bwrap driver for the caveat.
	...
	// Network is the egress policy. "deny" blocks all outbound;
