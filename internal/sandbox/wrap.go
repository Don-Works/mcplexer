...
// WrapCommand is Wrap plus per-invocation grants derived from the
// concrete program being launched: the executable chain (symlink
// target, shebang interpreter, Homebrew/npm/.app container) becomes
// readable, and workingDir/scratchDir become writable. Callers whose
// base Config cannot name the program up front — downstream MCP server
// spawns, where one wrapper serves many servers — use this so the
// deny-by-default profile still lets the child exec a non-system
// binary and write temp files. The base config is copied per call, so
// the wrapper stays reusable across spawns.
func (w *CommandWrapper) WrapCommand(
	program string, args []string, workingDir, scratchDir string,
) (string, []string, func()) {
	noop := func() {}
	if w == nil || !w.enabled {
		return program, args, noop
	}
	cfg := PrepareCommandConfig(w.cfg, program, workingDir, scratchDir)
	return wrapForPlatform(cfg, w.home, program, args, noop)
}
