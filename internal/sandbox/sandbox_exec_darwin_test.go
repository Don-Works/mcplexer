...
func TestWrapForPlatform_ProfileWriteFailureFailsClosed(t *testing.T) {
	program, args, cleanup := wrapForPlatformWithWriter(
		Config{}, "/Users/example", "/usr/bin/env", []string{"true"},
		func(string) (string, error) { return "", errors.New("disk full") },
	)
	cleanup()
	if program != sandboxWrapFailureProgram {
		t.Fatalf("program = %q, want fail-closed %q", program, sandboxWrapFailureProgram)
	}
	if len(args) != 0 {
		t.Fatalf("failure command must not receive original args: %v", args)
	}
}
...
// TestWrapCommandGrantsExecutableChainAndScratch — the per-spawn wrap
// used by downstream MCP spawns must materialise a profile that allows
// the concrete program (deny-by-default otherwise blocks exec of any
// non-system binary) and grants the scratch dir read-write.
func TestWrapCommandGrantsExecutableChainAndScratch(t *testing.T) {
	dir := t.TempDir()
	...
	w := NewCommandWrapper(Config{Network: NetworkHost})
	if !w.Enabled() {
		t.Skip("sandbox-exec unavailable on this host")
	}
	prog, args, cleanup := w.WrapCommand(bin, []string{"--stdio"}, "", scratch)
	defer cleanup()

	if prog != sandboxExecPath {
		t.Fatalf("program = %q, want %q", prog, sandboxExecPath)
	}
	if len(args) < 3 || args[0] != "-f" {
		t.Fatalf("args = %v, want -f <profile> <program> ...", args)
	}
	...
	if !strings.Contains(string(profile), strconv.Quote(resolvedBin)) {
		t.Fatalf("profile must allow the wrapped binary %q:\n%s", resolvedBin, profile)
	}
	resolvedScratch, err := filepath.EvalSymlinks(scratch)
	if err != nil {
		resolvedScratch = scratch
	}
	if !strings.Contains(string(profile), "file-write* (subpath "+strconv.Quote(resolvedScratch)+")") {
		t.Fatalf("profile must grant scratch read-write %q:\n%s", resolvedScratch, profile)
	}
	if args[len(args)-1] != "--stdio" || args[len(args)-2] != bin {
		t.Fatalf("wrapped args must end with original program+args, got %v", args)
	}
}
