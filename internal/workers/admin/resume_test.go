...
func seedDelegationRun(
	...
	run := &store.WorkerRun{
		WorkerID: w.ID, Status: "running", TriggerKind: triggerKind,
		ResultBranch: "mcplexer/delegation/" + name, ResultCommit: "trusted-" + name,
	}
...
func TestResumeOrphanedDelegations(t *testing.T) {
	...
	for _, id := range []string{manualRun, resumeRun, disabledRun} {
		...
		if r.ResultBranch == "" || r.ResultCommit == "" || r.ResultChanged {
			t.Errorf("run %s lost authoritative clean git result: branch=%q commit=%q changed=%v",
				id, r.ResultBranch, r.ResultCommit, r.ResultChanged)
		}
	}
