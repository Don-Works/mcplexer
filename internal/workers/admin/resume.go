...
func (s *Service) ResumeOrphanedDelegations(ctx context.Context) (int, error) {
	...
	for _, run := range runs {
		if err := s.finalizeOrphanedRun(ctx, run.ID, now); err != nil {
...
func (s *Service) finalizeOrphanedRun(ctx context.Context, runID string, now time.Time) error {
	return s.store.UpdateWorkerRunStatus(ctx, runID, store.WorkerRunFinalize{
