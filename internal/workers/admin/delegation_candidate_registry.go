...
func (s *Service) appendWorkerDelegationModelCandidates(
	...
	for _, workspaceID := range workspaceIDs {
		...
		for _, w := range workers {
			// Delegation workers are ephemeral execution records, not durable
			// model registrations. Feeding them back into the candidate pool
			// makes every historical ad-hoc model a future capacity candidate,
			// duplicates profile-backed rows, and keeps stale failed model IDs
			// selectable until the retention sweep disables the worker.
			if isEphemeralDelegationWorker(w) {
				continue
			}
...
func isEphemeralDelegationWorker(w *store.Worker) bool {
	if w == nil {
		return false
	}
	// Delegation metadata is the authoritative ephemeral marker. Names are
	// deliberately not consulted: durable registrations are allowed to use
	// the delegate-* namespace, including names that resemble generated rows.
	_, ok := parseDelegationMetadata(w.ParametersJSON)
	return ok
}
