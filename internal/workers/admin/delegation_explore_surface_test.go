...
// while a proven reviewed model remains the safer capacity default.
func TestCapacityListSurfacesExplorationForNewModel(t *testing.T) {
	...
	if proven.CapacityScore <= fresh.CapacityScore {
		t.Fatalf("proven capacity score %.2f must remain above fresh unreviewed score %.2f",
			proven.CapacityScore, fresh.CapacityScore)
	}
	if proven.Rank >= fresh.Rank {
		t.Fatalf("proven rank %d must be ahead of fresh experimental rank %d", proven.Rank, fresh.Rank)
	}
}
