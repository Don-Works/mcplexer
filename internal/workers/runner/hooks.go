...
func (r *Runner) runPostExecuteHook(
	...
	// Delegations promise a final report. Enforce that invariant natively so
	// the common path does not spend an extra execute_code dispatch or turn a
	// valid result into a sandbox-infrastructure failure. Caller-supplied post
	// hooks still run below for non-empty output.
	outcome = applyDelegationDeliverabilityGate(worker, outcome)
...
func applyDelegationDeliverabilityGate(worker *store.Worker, outcome loopOutcome) loopOutcome {
	if worker == nil || outcome.status != StatusSuccess || strings.TrimSpace(outcome.outputText) != "" {
		return outcome
	}
	var params map[string]json.RawMessage
	if err := json.Unmarshal([]byte(worker.ParametersJSON), &params); err != nil {
		return outcome
	}
	...
	outcome.status = StatusBlocked
	outcome.errorText = "post-execute deliverability gate blocked the run: delegation produced an empty final report"
	return outcome
}
