...
type loopState struct {
	...
	// worktree is present only for delegated runs using worktree isolation.
	// It stays live until finalize, output emission and persistence complete.
	worktree WorktreeLease
	// isolationRoot and claimedPaths are canonical runtime paths forwarded to
	// the gateway so inner tool calls are checked after dynamic JS evaluation.
	...
	// lastProgressEmit tracks the toolCallCount at which the most recent
	// progress mesh finding was emitted.
	lastProgressEmit int
...
func (r *Runner) runLoop(ctx context.Context, s *loopState) loopOutcome {
	for {
		...
		if maxOutput := s.lifetimeOutputCapSnapshot(); maxOutput > 0 && s.outputTokens >= maxOutput {
			return loopOutcome{
				status:     StatusCapExceeded,
				outputText: boundedCapExceededOutput(resp.Text, maxOutput),
				errorText:  fmt.Sprintf("output tokens (%d) exceeded cap (%d)", s.outputTokens, maxOutput),
			}
		}
		...
		// Emit periodic progress findings so the parent gets
		// intermediate results before the worker finishes.
		r.maybeEmitProgress(ctx, s)
	}
}

const maxCapExceededOutputBytes = 64 * 1024

func boundedCapExceededOutput(text string, tokenCap int) string {
	limit := maxCapExceededOutputBytes
	if tokenCap > 0 && tokenCap < limit/8 {
		limit = tokenCap * 8
		if limit < 4096 {
			limit = 4096
		}
	}
	return truncate(text, limit-len("…"))
}
