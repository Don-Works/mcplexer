...
// ErrWorkerConcurrent is returned before a WorkerRun is created when a
// concurrency_policy=skip worker already has a live run. Enforcing this in the
// Runner (not only the scheduler) covers mesh and manual dispatch as well.
var ErrWorkerConcurrent = errors.New("worker run already in progress")
...
func (r *Runner) RunWithOpts(ctx context.Context, workerID string, opts RunOpts) (runID string, err error) {
	...
	acquired, err := r.acquireWorkerSlot(ctx, worker)
	if err != nil {
		return "", err
	}
	if acquired {
		defer r.releaseWorkerSlot(worker.ID)
	}
	...
	// Register cleanup immediately after prepareRun transfers ownership of the
	// lease. Later defers (finalize, output persistence, browser release) run
	// first, so the checkout remains available throughout every terminal path.
	defer cleanupRunWorktree(state)
	var outcome loopOutcome
	// Register panic recovery and terminal finalization immediately after the
	// prepared run transfers lease ownership. Nothing below this point may
	// leave an unsnapshotted worktree or a permanently-running DB row.
	defer func() {
		if p := recover(); p != nil {
			stack := debug.Stack()
		...
		func() {
			defer func() {
				if p := recover(); p != nil {
					state.retainWorktree = state.worktree != nil
					fallbackCtx, fallbackCancel := context.WithTimeout(context.WithoutCancel(finalizeCtx), 5*time.Second)
					defer fallbackCancel()
					fallbackErr := r.persistFinalizationPanicFailure(fallbackCtx, worker, run, state, p)
					slog.Error("worker finalization panic; retained isolated worktree",
						"worker_id", worker.ID,
						"run_id", run.ID,
						"panic", fmt.Sprint(p),
		...
		// Tear down any per-session browser this worker spawned. Optional
		// capability: test dispatchers need not implement it.
	...
	ctx = WithWorkerRunCtx(ctx, WorkerRunCtx{
		...
		FilesystemRoot:    state.isolationRoot,
		WorkspacePath:     state.workspacePath,
		ClaimedPaths:      append([]string(nil), state.claimedPaths...),
		Branch:            state.branch,
	})
...
// acquireWorkerSlot returns acquired=false for queue-policy workers. For skip
// workers it combines a process-local keyed lock (the production daemon has
// one Runner shared by every trigger path) with the persisted running-row
// check, catching both concurrent goroutines and an already-observable run.
func (r *Runner) acquireWorkerSlot(ctx context.Context, worker *store.Worker) (bool, error) {
	if worker == nil || strings.EqualFold(strings.TrimSpace(worker.ConcurrencyPolicy), "queue") {
		return false, nil
	}
	r.workerSlotsMu.Lock()
	if r.workerSlots == nil {
		r.workerSlots = make(map[string]struct{})
	}
	if _, exists := r.workerSlots[worker.ID]; exists {
		r.workerSlotsMu.Unlock()
		return false, fmt.Errorf("%w: %s", ErrWorkerConcurrent, worker.ID)
	}
	r.workerSlots[worker.ID] = struct{}{}
	r.workerSlotsMu.Unlock()

	running, err := r.store.CountRunningWorkerRuns(ctx, worker.ID)
	if err != nil {
		r.releaseWorkerSlot(worker.ID)
		return false, fmt.Errorf("check running worker runs: %w", err)
	}
	if running > 0 {
		r.releaseWorkerSlot(worker.ID)
		return false, fmt.Errorf("%w: %s", ErrWorkerConcurrent, worker.ID)
...
func (r *Runner) releaseWorkerSlot(workerID string) {
	r.workerSlotsMu.Lock()
	delete(r.workerSlots, workerID)
	r.workerSlotsMu.Unlock()
}
...
func (r *Runner) prepareRun(ctx context.Context, worker *store.Worker, opts RunOpts, runID string) (*loopState, *store.WorkerRun, error) {
	policy, err := readDelegationIsolationPolicy(worker.ParametersJSON)
	if err != nil {
		return nil, nil, err
	}
	...
	defer func() {
		if lease != nil && !leaseTransferred {
			abandonWorktreeLease(runID, lease.RootPath(), lease)
		}
	}()
	if policy.required() {
		workspacePath, err = r.resolveWorkspacePathStrict(ctx, worker.WorkspaceID)
		if err != nil {
			return nil, nil, fmt.Errorf("prepare isolated delegation: %w", err)
		}
		lease, err = r.worktrees.Prepare(ctx, workspacePath, runID)
		if err != nil {
			return nil, nil, fmt.Errorf("prepare isolated delegation: %w", err)
		}
		workspacePath = lease.WorkspacePath()
	} else {
		workspacePath = r.resolveWorkspacePath(ctx, worker.WorkspaceID)
	}
	...
	var branch string
	if lease != nil {
		scope, scopeErr := prepareFilesystemScope(lease, policy.claims)
		if scopeErr != nil {
			return nil, nil, scopeErr
		}
		workspacePath = scope.WorkingDir()
		isolationRoot = scope.Root()
		claimedPaths = scope.Claims()
		branch = lease.Branch()
		if configureErr := lease.ConfigureSnapshotPolicy(claimedPaths, policy.reviewOnly); configureErr != nil {
			return nil, nil, fmt.Errorf("configure delegated snapshot policy: %w", configureErr)
		}
		systemPrompt = isolatedWorkerPreamble(lease, scope) + systemPrompt
	}
	listCtx := ctx
	if lease != nil {
		// Authenticate the exact filesystem scope before the model-facing tool
		// schemas are built. Production dispatchers use this context to select
		// the gateway-owned, non-introspecting isolated catalog.
		listCtx = WithWorkerRunCtx(listCtx, WorkerRunCtx{
			WorkerID:       worker.ID,
			RunID:          runID,
			WorkspaceID:    worker.WorkspaceID,
			FilesystemRoot: isolationRoot,
			WorkspacePath:  workspacePath,
			ClaimedPaths:   append([]string(nil), claimedPaths...),
			Branch:         branch,
		})
	}
	tools, err := r.dispatcher.ListTools(listCtx, parseAllowlistForWorker(worker.ID, worker.ToolAllowlistJSON))
	if err != nil {
	...
	adapter, err := r.adapter(models.Config{
		...
		Thinking:    workerModelThinking(worker),
	})
	if err != nil {
	...
	if pi := readProgressInterval(worker.ParametersJSON); pi > 0 {
		caps.ProgressInterval = pi
	}
	...
	state.worktree = lease
	state.isolationRoot = isolationRoot
	state.claimedPaths = claimedPaths
	state.branch = branch
	leaseTransferred = true
	return state, run, nil
}

// workerModelThinking reads the optional provider extension from parameters.
// Keeping this on the Worker (rather than a daemon-global env var) lets a
// deterministic triage worker disable expensive deep reasoning without
// changing unrelated openai_compat workers on the same gateway.
func workerModelThinking(worker *store.Worker) string {
	if worker == nil || worker.ModelProvider != models.ProviderOpenAICompat {
		return ""
	}
	params, err := parseParameters(worker.ParametersJSON)
	if err != nil {
		return ""
	}
	value, _ := params["model_thinking"].(string)
...
// resolveWorkspacePathStrict is the delegated-worktree variant of
// resolveWorkspacePath. Isolation cannot fall back to the daemon CWD or create
// a missing directory: either the configured workspace already exists and is
// usable, or the run fails before model execution.
func (r *Runner) resolveWorkspacePathStrict(ctx context.Context, workspaceID string) (string, error) {
	if workspaceID == "" || workspaceID == "global" {
		return "", errors.New("delegated worktree isolation requires a non-global workspace")
	}
	ws, err := r.store.GetWorkspace(ctx, workspaceID)
	if err != nil {
		return "", fmt.Errorf("get workspace %q: %w", workspaceID, err)
	}
	if ws == nil || strings.TrimSpace(ws.RootPath) == "" || ws.RootPath == "/" {
		return "", fmt.Errorf("workspace %q has no bounded root path", workspaceID)
	}
	root, err := existingDirectory(ws.RootPath)
	if err != nil {
		return "", fmt.Errorf("workspace %q root %q: %w", workspaceID, ws.RootPath, err)
	}
	return root, nil
}
...
func isWorkerLifecycle(m store.MeshMessage) bool {
	...
	// Periodic delegation progress findings are telemetry, not
	// conversation — keep them out of rendered mesh history windows.
	return strings.Contains(t, "delegation_progress")
}
...
func (r *Runner) finalize(
	...
	if state != nil && state.worktree != nil {
		...
		snapshot, snapshotErr := state.worktree.Snapshot(snapshotCtx)
		cancelSnapshot()
		if snapshotErr != nil {
			state.retainWorktree = true
			// Persist the owned recovery branch even when no trusted commit could
			// be produced. This keeps crash recovery discoverable and prevents
			// read-time fallback to untrusted model-reported branch/commit text.
			state.snapshot = WorktreeSnapshot{Branch: state.branch}
			outcome.status = StatusFailure
			if outcome.errorText != "" {
				outcome.errorText += "; "
			}
			outcome.errorText += fmt.Sprintf(
				"trusted worktree snapshot failed; retained branch %s at %s: %v",
				state.branch, state.isolationRoot, snapshotErr,
			)
		} else {
	...
	if r.persistRunFinalize(ctx, worker, run, state, outcome, finishedAt) {
		state.terminalPersisted = true
	} else if state.worktree != nil {
		state.retainWorktree = true
	}
...
func (r *Runner) persistRunFinalize(
	...
	fin := store.WorkerRunFinalize{
		...
		ResultBranch:       state.snapshot.Branch,
		ResultCommit:       state.snapshot.Commit,
		ResultChanged:      state.snapshot.Changed,
		HasGitResult:       state.worktree != nil,
	}
	...
	if fresh, ferr := r.store.GetWorkerRun(ctx, run.ID); ferr == nil && fresh != nil {
		...
		finalSnapshot.ResultBranch = fin.ResultBranch
		finalSnapshot.ResultCommit = fin.ResultCommit
		finalSnapshot.ResultChanged = fin.ResultChanged
	}
...
// persistFinalizationPanicFailure is the deliberately minimal last-resort DB
// write used when finalize itself panics. It avoids output/audit hooks and is
// called behind a second recover boundary, so a bad sink/store cannot crash
// the daemon. The worktree remains retained regardless of this result.
func (r *Runner) persistFinalizationPanicFailure(
	ctx context.Context,
	worker *store.Worker,
	run *store.WorkerRun,
	state *loopState,
	...
	defer func() {
		if p := recover(); p != nil {
			err = fmt.Errorf("fallback persistence panic: %v", p)
		}
	}()
	...
	finishedAt := r.clock.Now().UTC()
	return r.store.UpdateWorkerRunStatus(ctx, run.ID, store.WorkerRunFinalize{
		Status:             StatusFailure,
		FinishedAt:         finishedAt,
		InputTokens:        state.inputTokens,
		OutputTokens:       state.outputTokens,
		...
		ToolCallsCount:     state.toolCallCount,
		OutputText:         "",
		Error:              fmt.Sprintf("worker finalization panic: %v", panicValue),
		MeshMessageIDsJSON: "[]",
		AuditRecordIDsJSON: "[]",
		BillingModel:       state.billingModel,
		SubscriptionBucket: state.subscriptionBucket,
...
// maybeEmitProgress checks whether enough tool calls have elapsed since
// the last progress emission and sends a compact progress finding.
func (r *Runner) maybeEmitProgress(ctx context.Context, s *loopState) {
	caps := s.capsSnapshot()
	interval := caps.ProgressInterval
	if interval <= 0 || r.mesh == nil {
		return
	}
	if s.toolCallCount-s.lastProgressEmit < interval {
		return
	}
	s.lastProgressEmit = s.toolCallCount
	elapsed := r.clock.Now().Sub(s.startedAt).Truncate(time.Second)
	content := fmt.Sprintf(
		"## Progress: Worker %s (%d tool calls, %s elapsed)\nTokens: %d in / %d out | Cost: $%.4f | Iterations: %d",
		s.worker.ID, s.toolCallCount, elapsed,
		s.inputTokens, s.outputTokens, s.costUSD, s.iteration,
	)
	tags := "delegation_progress,worker:" + safeTag(s.worker.ID)
	if s.runID != "" {
		tags += ",run:" + safeTag(s.runID)
	}
	s.appendMeshID(r.emitSignal(ctx, s.worker.ID, s.runID, MeshOutbound{
		Kind:     "finding",
		Content:  content,
		Tags:     tags,
		Priority: "low",
...
// readProgressInterval extracts the progress_interval from worker
// parameters JSON. Delegations nest it inside the "_mcplexer_delegation"
// metadata object; a top-level key remains the escape hatch for plain
// workers. Returns 0 when not set or invalid.
func readProgressInterval(paramsJSON string) int {
	if paramsJSON == "" {
		return 0
	}
	var obj map[string]any
	if err := json.Unmarshal([]byte(paramsJSON), &obj); err != nil {
		return 0
	}
	if meta, ok := obj["_mcplexer_delegation"].(map[string]any); ok {
		if n := intFromJSON(meta["progress_interval"]); n > 0 {
			return n
		}
	}
	return intFromJSON(obj["progress_interval"])
}
