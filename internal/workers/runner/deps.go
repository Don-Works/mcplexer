...
type Caps struct {
	...
	ProgressInterval int           // emit progress every N tool calls; 0 disables (delegations opt in via metadata)
}
...
func (c Caps) applyDefaults() Caps {
	...
	// ProgressInterval deliberately has NO default: progress findings are
	// opt-in (delegations enable them via metadata). Defaulting here would
	// turn every scheduled worker into a delegation_progress emitter.
	return c
}
...
type Runner struct {
	...
	// workerSlots enforces concurrency_policy=skip at the Runner boundary,
	// shared by schedule, mesh, and manual dispatch. The scheduler's DB check
	// remains a cheap preflight; this keyed guard closes the in-process race and
	// prevents non-scheduler entrypoints from bypassing the policy.
	workerSlotsMu sync.Mutex
	workerSlots   map[string]struct{}
}
...
type Deps struct {
	...
	CLIToolCounter CLIToolCallCounter
	// Worktrees creates isolated git checkouts for delegated runs. Nil uses
	// the production git-worktree manager rooted outside the source checkout.
	Worktrees WorktreeManager
}
...
func New(deps Deps) *Runner {
	...
	if r.worktrees == nil {
		r.worktrees = NewGitWorktreeManager("")
	}
	return r
}
