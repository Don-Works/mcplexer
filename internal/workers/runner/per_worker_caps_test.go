...
func TestRun_PerWorkerMaxOutputTokensCapsTerminalResponse(t *testing.T) {
	db := newTestStore(t)
	wsID, scopeID := setupFKs(t, db)
	w := sampleWorker(wsID, scopeID)
	w.MaxOutputTokens = 64
	...
	largeReport := strings.Repeat("evidence ", 20_000)
	adapter := &fakeAdapter{responses: []models.SendResponse{{
		Text:         largeReport,
		OutputTokens: 128,
	...
	if !strings.Contains(run.Error, "output tokens") {
		t.Fatalf("error = %q, want output-cap root cause", run.Error)
	}
	if run.OutputText == "" || !strings.HasPrefix(run.OutputText, "evidence") {
		t.Fatalf("partial output evidence was not preserved: %q", run.OutputText)
	}
	if len(run.OutputText) > 4096 {
		t.Fatalf("cap evidence length = %d, want bounded to <= 4096 bytes", len(run.OutputText))
	}
}
