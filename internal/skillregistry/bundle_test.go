...
func TestFetchBundleForEntryNeverFallsBackAcrossScopes(t *testing.T) {
	reg, _ := newTestRegistry(t)
	ctx := context.Background()
	workspace := "bundle-exact-workspace"
	name := "exact-bundle"
	globalBody := sampleBody(name, "Use when testing the global exact bundle.")
	globalBundle := buildBundle(t, name, map[string]string{
		"SKILL.md": globalBody, "scope.txt": "global",
	})
	if _, err := reg.Publish(ctx, skillregistry.PublishOptions{
		Name: name, Body: globalBody, Bundle: globalBundle,
	}); err != nil {
		t.Fatalf("publish global: %v", err)
	}
	workspaceBody := sampleBody(name, "Use when testing the workspace exact bundle.")
	workspaceBundle := buildBundle(t, name, map[string]string{
		"SKILL.md": workspaceBody, "scope.txt": "workspace",
	})
	if _, err := reg.Publish(ctx, skillregistry.PublishOptions{
		Name: name, Body: workspaceBody, Bundle: workspaceBundle, WorkspaceID: &workspace,
	}); err != nil {
		t.Fatalf("publish workspace: %v", err)
	}

	scope := store.SkillScope{WorkspaceIDs: []string{workspace}}
	entry, err := reg.Get(ctx, scope, name, skillregistry.VersionRef{Version: 1})
	if err != nil || entry.WorkspaceID == nil || *entry.WorkspaceID != workspace {
		t.Fatalf("resolve workspace entry: entry=%+v err=%v", entry, err)
	}
	bundle, _, err := reg.FetchBundleForEntry(ctx, entry)
	if err != nil || !bytes.Equal(bundle, workspaceBundle) {
		t.Fatalf("exact workspace bundle mismatch: size=%d err=%v", len(bundle), err)
	}

	if err := reg.SoftDelete(ctx, &workspace, name, 1); err != nil {
		t.Fatalf("delete selected workspace row: %v", err)
	}
	if _, _, err := reg.FetchBundleForEntry(ctx, entry); !errors.Is(err, store.ErrNotFound) {
		t.Fatalf("stale exact entry fell back instead of ErrNotFound: %v", err)
	}
	// Ordinary visibility resolution now falls back to global, proving the
	// exact API above did not simply return whichever visible row survived.
	bundle, _, err = reg.FetchBundle(ctx, scope, name, skillregistry.VersionRef{Version: 1})
	if err != nil || !bytes.Equal(bundle, globalBundle) {
		t.Fatalf("visibility fallback control check: size=%d err=%v", len(bundle), err)
	}
}
