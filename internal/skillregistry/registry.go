...
func New(s store.SkillRegistryStore) *Registry {
	return &Registry{store: s, indexes: map[string]*scopedIndex{}}
}
...
// applying the same workspace-shadowing rule as GetSkillRegistryHead: a
// workspace-pinned row of that version shadows the global row of the same
// version. Honours scope.IncludeAll (admin) by delegating to a
...
func (r *Registry) resolveVersionInScope(
	...
	for i := range versions {
		...
		// Mirror the "(workspace_id IS NULL) ASC" shadow ordering used by
		// GetSkillRegistryHead: a workspace-scoped row wins over global.
		if best == nil || (best.WorkspaceID == nil && e.WorkspaceID != nil) {
			row := e
			best = &row
		}
	}
...
func scopeKey(scope store.SkillScope) string {
	...
	// Stable order so different goroutines hit the same cache key.
	ids := append([]string(nil), scope.WorkspaceIDs...)
	sort.Strings(ids)
