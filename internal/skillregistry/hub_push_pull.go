...
func (r *Registry) Push(ctx context.Context, scope store.SkillScope, name string, ref VersionRef) (*PushResult, error) {
	...
	if err := checkSyncPortableEntry(entry); err != nil {
		return nil, fmt.Errorf("push %s: %w", entry.Name, err)
	}
...
func (r *Registry) DryRunPull(ctx context.Context, scope store.SkillScope, hubEntry *HubPullEntry) (*DryRunResult, error) {
	...
	if hubEntry == nil {
		return nil, errors.New("skillregistry: pull entry is nil")
	}
	if hubEntry.Body != "" {
		if err := CheckSyncPortableBody(hubEntry.Name, hubEntry.Body); err != nil {
			return nil, fmt.Errorf("pull %s: %w", hubEntry.Name, err)
		}
	}
	res := &DryRunResult{
		Name:    hubEntry.Name,
...
func (r *Registry) Pull(ctx context.Context, scope store.SkillScope, opts PullOptions) (*PullResult, error) {
	...
	if !errors.Is(err, store.ErrNotFound) {
		if hubEntry.Body == "" {
			if err := checkSyncPortableEntry(local); err != nil {
				return nil, fmt.Errorf("pull %s: %w", opts.Name, err)
			}
		}
		if hubEntry.ContentHash == "" {
	...
	// Preserve legacy body-less previews, but never let a commit bypass the
	// inbound body check. Callers carrying a package must pass its raw body.
	if hubEntry.Body == "" {
		return nil, errors.New("skillregistry: pull body required for commit")
	}
	published, err := r.Publish(ctx, PublishOptions{
		Name:               opts.Name,
		Body:               hubEntry.Body,
		Author:             "hub-pull",
		WorkspaceID:        nil,
	...
	return &PullResult{
		...
		BundleSHA:   published.BundleSHA256,
		BundleSize:  published.BundleSize,
	}, nil
}
...
func (r *Registry) ListIndexEntries(ctx context.Context, scope store.SkillScope) ([]IndexEntry, error) {
	...
	for _, e := range heads {
		if err := checkSyncPortableEntry(&e); err != nil {
			if errors.Is(err, ErrCompositionNotPortable) {
				continue
			}
			return nil, fmt.Errorf("index %s: %w", e.Name, err)
		}
