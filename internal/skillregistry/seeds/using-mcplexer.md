...
1. `mcpx__search_tools` — find any callable. Start cheap with summary mode: `{queries:["create task"], namespaces:["task"]}`. The common singular form `{query:"create task"}` is accepted too. Fetch one exact signature with `{tool:"task__create"}`. Use `detail:"full"` only for a narrow query/namespace when you need several signatures at once.
...
The skill registry is deliberately in the `mcpx` namespace. The separate `skill` namespace records run telemetry (`skill.run_start`, `skill.phase`, `skill.run_complete`); `skill.get(...)` is not a registry lookup.
