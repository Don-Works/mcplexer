...
func TestParse_ManifestExtra(t *testing.T) {
	...
	if got, want := m.Includes, []SkillInclude{{
		ID: "shared-contract", Skill: "neutral-contract", Scope: "global", Version: 3,
		ContentHash: strings.Repeat("a", 64), Section: "rules",
...
func TestValidateExtra_HappyAndAbsent(t *testing.T) {
	cases := []struct {
		...
		{
			"content-pinned include",
			ManifestExtra{Includes: []SkillInclude{{
				ID: "shared-contract", Skill: "neutral-contract", Scope: "global",
				Version: 2, ContentHash: strings.Repeat("a", 64), Section: "rules",
			}}},
		},
	}
...
func TestValidateExtra_Failures(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			ManifestExtra{Includes: []SkillInclude{{
				ID: "x", Skill: "neutral", Version: 1, ContentHash: strings.Repeat("a", 64),
			}}},
			ErrInvalidInclude,
		},
		{
			"include floating version",
			ManifestExtra{Includes: []SkillInclude{{
				ID: "x", Skill: "neutral", Scope: "global", ContentHash: strings.Repeat("a", 64),
			}}},
			ErrInvalidInclude,
		},
		{
			"include short hash",
			ManifestExtra{Includes: []SkillInclude{{
				ID: "x", Skill: "neutral", Scope: "global", Version: 1, ContentHash: "abc",
			}}},
			ErrInvalidInclude,
		},
		{
			...
			ManifestExtra{Includes: []SkillInclude{
				{ID: "x", Skill: "one", Scope: "global", Version: 1, ContentHash: strings.Repeat("a", 64)},
				{ID: "x", Skill: "two", Scope: "same", Version: 2, ContentHash: strings.Repeat("b", 64)},
			}},
			ErrInvalidInclude,
		},
		{
			"too many includes",
			ManifestExtra{Includes: func() []SkillInclude {
				out := make([]SkillInclude, MaxSkillIncludes+1)
				for i := range out {
					out[i] = SkillInclude{ID: fmt.Sprintf("edge-%d", i), Skill: "neutral", Scope: "global", Version: 1, ContentHash: strings.Repeat("a", 64)}
				}
				return out
			}()},
			ErrInvalidInclude,
...
func TestMarshalExtra_Roundtrip(t *testing.T) {
	want := ManifestExtra{
		...
		Includes: []SkillInclude{{
			ID: "shared", Skill: "neutral", Scope: "same", Version: 3,
			ContentHash: strings.Repeat("c", 64), Section: "rules",
		}},
	}
	...
	if !strings.Contains(string(data), `"includes"`) {
		t.Errorf("MarshalExtra missing includes key: %s", data)
	}
