...
type DockerObservation struct {
	...
	// PortInventoryAttempted distinguishes a deliberately skipped host-wide
	// sample from an attempted command that failed. A skipped sibling source
	// must not emit a false "inventory unavailable" incident.
	PortInventoryAttempted bool
}
...
func collectDockerObservation(
	...
	includePortInventory bool,
) *DockerObservation {
	if src.Kind == store.LogSourceKindJournald {
		return nil
	}
	obs := &DockerObservation{}
	if includePortInventory {
		obs.PortInventoryAttempted = true
