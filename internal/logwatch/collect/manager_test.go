...
func TestPortInventoryIsHostScopedAndCadenceBounded(t *testing.T) {
	now := time.Date(2026, 7, 17, 12, 0, 0, 0, time.UTC)
	m := NewManager(nil, nil, nil, &fakeRunner{})
	m.now = func() time.Time { return now }
	first := &store.LogSource{ID: "s1", RemoteHostID: "host-1", Kind: store.LogSourceKindDocker}
	sibling := &store.LogSource{ID: "s2", RemoteHostID: "host-1", Kind: store.LogSourceKindCompose}
	if !m.claimPortInventory(first) {
		t.Fatal("first due Docker source must claim host inventory")
	}
	if m.claimPortInventory(sibling) {
		t.Fatal("sibling source must not repeat host-wide inventory")
	}
	now = now.Add(4 * time.Minute)
	if m.claimPortInventory(first) {
		t.Fatal("host inventory ran before cadence elapsed")
	}
	now = now.Add(2 * time.Minute)
	if !m.claimPortInventory(sibling) {
		t.Fatal("host inventory did not re-arm after cadence")
	}
	if !m.claimPortInventory(&store.LogSource{ID: "s3", RemoteHostID: "host-2", Kind: store.LogSourceKindSwarm}) {
		t.Fatal("a different host needs its own inventory")
	}
	if m.claimPortInventory(&store.LogSource{ID: "journal", RemoteHostID: "host-3", Kind: store.LogSourceKindJournald}) {
		t.Fatal("journald source must not run Docker inventory")
	}
}

func TestSkippedPortInventoryIsNotAnUnavailableAttempt(t *testing.T) {
	ctx := withPortInventory(context.Background(), false)
	if portInventoryEnabled(ctx) {
		t.Fatal("manager-elected sibling pull should skip host inventory")
	}
	if !portInventoryEnabled(context.Background()) {
		t.Fatal("direct/integration pulls must preserve the historical default")
	}
}
...
func (r *concurrencyRunner) Pull(ctx context.Context, _ *store.RemoteHost, _ sshx.Credential, src *store.LogSource, _, _ time.Time, _ string) (PullResult, error) {
...
func testHostAndScope() (*store.RemoteHost, *store.AuthScope) {
	return &store.RemoteHost{ID: "h1", Name: "prod-1", SSHUser: "logwatch", SSHHost: "192.0.2.1", SSHPort: 22, AuthScopeID: "sc1", Enabled: true},
