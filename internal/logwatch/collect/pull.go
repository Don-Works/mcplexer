...
func (m *Manager) ingestPull(
	...
	if result.Docker != nil {
		...
		if result.Docker.PortInventoryAttempted {
			var portLines []Line
...
type portInventoryContextKey struct{}

func withPortInventory(ctx context.Context, enabled bool) context.Context {
	return context.WithValue(ctx, portInventoryContextKey{}, enabled)
}

// portInventoryEnabled defaults true for direct pullSource/integration calls;
// Manager.tick explicitly elects at most one source per host/cadence.
func portInventoryEnabled(ctx context.Context) bool {
	enabled, ok := ctx.Value(portInventoryContextKey{}).(bool)
	return !ok || enabled
}
...
func (sshRunner) Pull(ctx context.Context, host *store.RemoteHost, cred sshx.Credential, src *store.LogSource, logSince, eventsSince time.Time, cursor string) (PullResult, error) {
	...
	defer client.Close()
	observation := collectDockerObservation(ctx, client, src, eventsSince, portInventoryEnabled(ctx))
