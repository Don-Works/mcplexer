...
// Docker's published-port inventory is host-wide, not source-specific. Running
// it for every container source turned N sources at a 2m cadence into N×720
// redundant remote commands/day. Sample once per host on a modest cadence;
// per-source runtime/restart/log collection remains unchanged.
const portInventoryInterval = 5 * time.Minute
...
type Manager struct {
	...
	lastPortInventory map[string]time.Time // host id → last host-wide inventory attempt
...
func NewManager(st Store, secrets SecretReader, sink Sink, runner Runner) *Manager {
	...
	return &Manager{
		...
		hostPorts: map[string]string{}, lastPortInventory: map[string]time.Time{},
...
func (m *Manager) tick(ctx context.Context) {
	...
	for _, src := range sources {
		...
		m.markRun(src.ID)
		collectPorts := m.claimPortInventory(src)
		g.Go(func() error {
			m.pullOne(withPortInventory(ctx, collectPorts), src)
			return nil
		})
	}
	_ = g.Wait()
...
func (m *Manager) pruneState(sources []*store.LogSource) {
	...
	for id := range m.lastPortInventory {
		if _, ok := liveHosts[id]; !ok {
			delete(m.lastPortInventory, id)
		}
	}
}
...
func (m *Manager) claimPortInventory(src *store.LogSource) bool {
	if src == nil || src.Kind == store.LogSourceKindJournald || src.Kind == store.LogSourceKindFile {
		return false
	}
	now := m.now().UTC()
	m.mu.Lock()
	defer m.mu.Unlock()
	last := m.lastPortInventory[src.RemoteHostID]
	if !last.IsZero() && now.Sub(last) < portInventoryInterval {
		return false
	}
	m.lastPortInventory[src.RemoteHostID] = now
	return true
}
