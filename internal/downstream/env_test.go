...
func TestBrowserSessionEnvPropagatesLogicalOwner(t *testing.T) {
	if got := browserSessionEnv(InstanceKey{ServerID: "brw"}); got != nil {
		t.Fatalf("shared instance env = %#v, want nil", got)
	}
	env := browserSessionEnv(InstanceKey{ServerID: "brw", SessionID: "worker:agent-42"})
	if got := env["MCPLEXER_BROWSER_SESSION_ID"]; got != "worker:agent-42" {
		t.Fatalf("browser session env = %q, want worker:agent-42", got)
	}
}
