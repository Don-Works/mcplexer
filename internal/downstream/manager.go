...
func (m *Manager) createInstance(
	...
	env := MergeEnv(os.Environ(), browserSessionEnv(key), authEnv)
...
// browserSessionEnv propagates the logical browser-agent identity into a
// session-isolated stdio downstream. Browser adapters such as brw use it to
// lease a tab consistently even when MCPlexer replaces their disposable proxy
// process. SessionID is populated only for browser-class downstreams.
func browserSessionEnv(key InstanceKey) map[string]string {
	if key.SessionID == "" {
		return nil
	}
	return map[string]string{"MCPLEXER_BROWSER_SESSION_ID": key.SessionID}
}
