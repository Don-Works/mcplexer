...
# The hook lives at /v1/hooks/pretool (NOT /api/v1/...) and is intentionally
# unauthenticated — the curl shim Claude Code installs has no access to the
# api-key. Production rejects every non-loopback caller, so the integration
# harness must execute hook requests inside the target container. Calling a
# published host port would arrive from Docker's bridge gateway and correctly
# receive HTTP 403. container_for comes from test/integration/lib.sh.
sg_hook_url() {
    printf 'http://127.0.0.1:3333/v1/hooks/pretool'
}
...
sg_hook_request() {
    ...
    if [ -z "$container" ]; then
        printf 'unknown integration node URL: %s\n' "$node" >&2
        return 1
    fi

    if [ "$method" = "GET" ]; then
        docker exec "$container" curl -s --max-time "$timeout" \
            -X GET "$(sg_hook_url)" 2>/dev/null || true
        return
    fi

    printf '%s' "$body" | docker exec -i "$container" \
        curl -s --max-time "$timeout" \
        -X "$method" "$(sg_hook_url)" \
        -H 'Content-Type: application/json' \
        --data-binary @- 2>/dev/null || true
}

sg_hook_request_status() {
    local node="$1"
    local timeout="$2"
    local method="$3"
    local body="${4:-}"
    ...
    if [ "$method" = "GET" ]; then
        status=$(docker exec "$container" curl -s --max-time "$timeout" \
            -o /dev/null -w '%{http_code}' -X GET "$(sg_hook_url)" \
            2>/dev/null || true)
    else
        status=$(printf '%s' "$body" | docker exec -i "$container" \
...
sg_post_hook() {
    ...
    sg_hook_request "$url" "$timeout" POST "$body"
}
...
sg_post_hook_status() {
    ...
    local timeout="$2"
    local body="$3"
    sg_hook_request_status "$url" "$timeout" POST "$body"
}
...
scenario_shellguard_hook_surface() {
    ...
    # A request through the published host port must stay forbidden. This
    # proves the harness's in-container path below is not weakening the
    # production boundary or relying on a forwarding-header bypass.
    local host_status
    host_status=$(curl -s --max-time 3 -o /dev/null -w '%{http_code}' \
        -X POST "$NODE_A/v1/hooks/pretool" \
        -H 'Content-Type: application/json' \
        --data-binary "$edit_body" 2>/dev/null || true)
    if [ "$host_status" = "403" ]; then
        pass "17.0 published hook port rejects non-loopback caller"
    else
        fail "17.0 published hook boundary" "got status=$host_status, want 403"
    fi

    body=$(sg_hook_request "$NODE_A" 3 POST "$edit_body")
    ...
    body=$(sg_hook_request "$NODE_A" 3 POST "$missing_body")
    ...
    status=$(sg_hook_request_status "$NODE_A" 3 GET)
    if [ "$status" = "405" ]; then
...
rm /etc" '{session_id:"s18.5", hook_event_name:"PreToolUse",
    ...
    body=$(sg_hook_request "$NODE_A" 2 POST "$nl_body")
