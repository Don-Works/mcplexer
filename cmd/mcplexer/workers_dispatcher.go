...
func (d *toolDispatcher) dispatchBuiltin(ctx context.Context, call runner.ToolCallRequest) (runner.ToolCallResult, error) {
	...
	ctx = d.withWorkerAllowlist(ctx, call)
...
func (d *toolDispatcher) withWorkerAllowlist(ctx context.Context, call runner.ToolCallRequest) context.Context {
	...
	if err != nil || worker == nil {
		...
		// entrypoint.
		return gateway.WithWorkerCapabilityProfile(ctx, denyEverythingCapabilityProfile())
	}
	ctx = gateway.WithWorkerToolAllowlist(
		ctx,
		parseAllowlistPatternsForWorker(worker.ID, worker.ToolAllowlistJSON),
	)
	ctx = gateway.WithWorkerCapabilityProfile(
		ctx,
		parseCapabilityProfileForWorker(worker.ID, worker.CapabilityProfileJSON),
	)
	return gateway.WithWorkerWorkspaceAccess(
		ctx,
		worker.WorkspaceID,
		d.workerWorkspaceGrants(ctx, worker),
	)
}
