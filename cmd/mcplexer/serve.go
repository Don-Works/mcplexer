...
func buildServerDeps(ctx context.Context, cfg *Config, db *sqlite.DB, settingsSvc *config.SettingsService) (*serverDeps, error) {
	...
	// paths (~/.ssh, ~/.aws, etc.) are inaccessible. The base config
	// here is deliberately bare: the profile is deny-by-default, and
	// per-spawn grants (the server binary's executable chain plus a
	// TMPDIR scratch dir) are derived per command by
	// Instance.start via CommandWrapper.WrapCommand. Off by default
	...
	if brainCfg.Enabled {
		...
		// On darwin, fsnotify's kqueue backend pins one fd per file inside
		// every watched directory, so a populated brain tree bloats the fd
		// table toward exhaustion; poll mtimes there instead. External-edit
		// latency is the sweep interval — daemon writes index in-process.
		// MCPLEXER_BRAIN_WATCH=fsnotify|poll overrides the platform default.
		var w brain.DirWatcher
		var wErr error
		watchMode := os.Getenv("MCPLEXER_BRAIN_WATCH")
		if watchMode == "" && runtime.GOOS == "darwin" {
			watchMode = "poll"
		}
		if watchMode == "poll" {
			w = brain.NewPoller(ix, 0)
		} else {
			w, wErr = brain.NewWatcher(ix, 0)
		}
		if wErr != nil {
	...
	workerAdminSvc.SetFileClaimStore(db)
	// Feature 6 (context sharing): auto-inject recalled memory + recent
	// mesh activity into worker prompts so delegates skip cold-start
	// re-exploration.
	if d.memorySvc != nil {
		workerAdminSvc.SetMemoryRecaller(d.memorySvc)
	}
