...
func TestAutoInstallLogWatch(t *testing.T) {
	...
	if !strings.Contains(w.PreExecuteScript, `s.pending_templates === 0`) {
		t.Fatalf("gate must use the durable pending queue: %q", w.PreExecuteScript)
	}
	...
	if strings.Count(w.ToolAllowlistJSON, `"monitoring__commit_triage"`) != 1 ||
		strings.Count(w.ToolAllowlistJSON, `"monitoring__triage_effect"`) != 1 {
		t.Fatalf("worker must have commit + effect grants exactly once: %s", w.ToolAllowlistJSON)
	}
	if strings.Contains(w.ToolAllowlistJSON, `"task__`) {
		t.Fatalf("fresh worker must not spend calls on task primitives: %s", w.ToolAllowlistJSON)
	}
	if w.PostExecuteScript != logWatchPostGate || !strings.Contains(w.PostExecuteScript, "triage_effect") {
		t.Fatalf("effect-based postcondition missing: %q", w.PostExecuteScript)
	}
	if !strings.Contains(w.ParametersJSON, `"model_thinking":"disabled"`) {
		t.Fatalf("log-watch must disable deep thinking without changing its model: %s", w.ParametersJSON)
	}
...
func TestAutoInstallLogWatchConvergesLegacySafetyConfig(t *testing.T) {
	...
	legacyTools := `["monitoring__digest","task__list","mesh__send"]`
	legacyParams := `{"operator_setting":"preserved"}`
	...
	_, err = workers.Update(ctx, workersadmin.UpdateInput{ID: got.Worker.ID,
		...
		ToolAllowlistJSON: &legacyTools,
		ParametersJSON:    &legacyParams,
	...
	if w.PreExecuteScript != logWatchGate || w.PostExecuteScript != logWatchPostGate ||
	...
	if strings.Count(w.ToolAllowlistJSON, `"monitoring__commit_triage"`) != 1 ||
		strings.Count(w.ToolAllowlistJSON, `"monitoring__triage_effect"`) != 1 ||
		!strings.Contains(w.ToolAllowlistJSON, `"mesh__send"`) {
		t.Fatalf("legacy allowlist did not gain durable triage tools while preserving operator tools: %s", w.ToolAllowlistJSON)
	}
	if !strings.Contains(w.ParametersJSON, `"operator_setting":"preserved"`) ||
		!strings.Contains(w.ParametersJSON, `"model_thinking":"disabled"`) {
		t.Fatalf("legacy parameters did not preserve operator values and gain model_thinking: %s", w.ParametersJSON)
	}
}
