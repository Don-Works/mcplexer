...
// TestBridgeStdioToSocket_StdinEOFDoesNotWaitForPeerClose guards the clean
// shutdown path against a daemon that observes the write-side EOF but keeps
// its read side open. The bridge may briefly drain a final response, but it
// must return before an outer MCP process manager gives up on shutdown.
func TestBridgeStdioToSocket_StdinEOFDoesNotWaitForPeerClose(t *testing.T) {
	dir := shortSockDir(t)
	...
	go func() {
		done <- bridgeStdioToSocket(ctx, stdinR, io.Discard, sockPath)
	}()
	...
	case err := <-done:
		...
		if elapsed := time.Since(start); elapsed > time.Second {
			t.Fatalf("bridge took %v after stdin EOF", elapsed)
		}
	case <-time.After(1500 * time.Millisecond):
		t.Fatal("bridge did not return promptly after stdin EOF")
	}

	select {
	case <-eofSeen:
	case <-time.After(time.Second):
		t.Fatal("daemon peer did not observe write-side EOF")
	}
}
