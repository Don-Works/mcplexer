...
// stdinDrainTimeout gives the daemon a short grace period to flush a final
// response after the client closes stdin. Keep this comfortably below the
// outer process/test shutdown budgets: using the same two-second duration at
// both layers made clean EOF race the caller's timeout on loaded Linux CI.
const stdinDrainTimeout = 500 * time.Millisecond
