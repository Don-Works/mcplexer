...
export interface DelegationModelStat {
  ...
  cost_usd: number
  unknown_cost_runs?: number
  unknown_success_runs?: number
  duration_ms: number
  avg_duration_ms: number
  unknown_duration_ms?: number
  operational_duration_ms?: number
  operational_failures?: number
  dispatch_failures?: number
  budget_failures?: number
  deliverability_failures?: number
  quality_success?: number
...
export interface DelegationModelCapacity {
  ...
  cost_usd: number
  avg_duration_ms: number
  // exploration_bonus is an informational UCB-style signal that is deliberately
  // excluded from production capacity_score. exploring marks a fresh or rare
  // model worth evaluating explicitly, not a model silently boosted into the
