...
let lastSyntheticNotificationID = 0

export function nextSyntheticNotificationID(now = Date.now()): number {
  const timeBased = -Math.trunc(now) * 1000
  lastSyntheticNotificationID =
    lastSyntheticNotificationID === 0
      ? timeBased
      : Math.min(timeBased, lastSyntheticNotificationID - 1)
  return lastSyntheticNotificationID
}
...
export function useSignalStream() {
  useEffect(() => {
    ...
    const unsubLive = subscribeEvent('notifications', (data) => {
      ...
      // the StoredEvent). Use a unique negative transient id so same-millisecond
      // events cannot share read state; persisted rows always use positive ids.
      const synthetic: StoredNotification = {
        id: nextSyntheticNotificationID(),
...
export function useSignal() {
  ...
  return {
    ...
    openLink: (link: string | undefined) => {
      const safe = safeNotificationPath(link)
