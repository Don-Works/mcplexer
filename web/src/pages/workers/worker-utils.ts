...
import type { DelegationContext, WorkerRunStatus, WorkerSummary } from '@/api/workers'
...
export function delegationStatusBadgeClass(status: string): string {
  ...
  if (status === 'blocked') return statusBadgeClass('blocked')
...
export function delegationNeedsReview(delegation: DelegationContext): boolean {
  return delegation.status === 'needs_review'
}

export function delegationBlocked(delegation: DelegationContext): boolean {
