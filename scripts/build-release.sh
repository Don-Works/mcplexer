...
# ZIP runs from inside WORK_DIR, so keep one canonical output path instead of
# assuming DIST_DIR is a single repository-relative directory. This also makes
# absolute CI/temp paths and paths containing spaces behave identically.
...
bash scripts/generate-third-party-licenses.sh "$WORK_DIR/THIRD_PARTY_LICENSES"

LDFLAGS="-s -w -X main.buildVersion=${VERSION} -X main.buildCommit=${COMMIT} -X github.com/don-works/mcplexer/internal/gateway.buildVersion=${VERSION}"
...
build_one() {
  ...
  cp -R "$WORK_DIR/THIRD_PARTY_LICENSES" "$pkg/"
...
bash scripts/write-release-checksums.sh "$DIST_DIR"

echo "==> Verifying release archives"
scripts/release-smoke.sh --dist "$DIST_DIR" --version "$VERSION" --commit "$COMMIT" --structure-only
