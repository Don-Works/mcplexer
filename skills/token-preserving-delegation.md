...
2. **Don't race your worker.** 19 of 100 delegations were reviewed "superseded" — the parent landed the same fix itself while the worker ran, paying twice (one worker burned 2.7M tokens and 25 minutes for a 74 "partial/superseded"). Once you delegate a slice: declare its paths both in the linked task's `meta.touches_files` and the delegation's `touches_files`, then either `mcpx__wait_for_delegation` or work a DIFFERENT slice. If the worker is too slow or off-track, cancel it explicitly — never silently duplicate it.
...
5. Spawn workers with `mcpx__delegate_worker`.
   ...
   - Leave `worker_isolation` omitted to use the secure `worktree` default for API-backed repository work. Use `worker_isolation: "none"` only as an explicit trusted escape hatch; it is required for CLI-backed delegates until a capability-required per-run relay is available.
   - Pass concrete workspace-relative paths in `touches_files` for implementation slices. API-backed workers receive only the gateway-owned `mcpx__workspace_read_file`, `mcpx__workspace_list_directory`, `mcpx__workspace_write_file`, and `mcpx__workspace_edit_file` operations. Traversal, unresolved expansion/globs, symlink escapes, Git metadata, and writes outside declared claims fail closed.
   - All worktree-isolated CLI delegates fail before checkout creation, secret resolution, or model launch. API-backed isolated workers do not receive arbitrary local filesystem, shell, or opaque downstream tools, so the parent must run builds and tests that require a command runner.
   ...
   - Memory is available by default for execute workers (via the three-tool surface): the worker preamble directs persistence to the `memory` namespace; use `mcpx__search_tools` (or harness alias) for "memory", then `mcpx__call_tool` for one independent operation or `execute_code` for related/derived operations with `memory.save({...})` / `memory.recall({...})` / `memory.list` (see docs/memory.md and CLAUDE.md for exact call forms and harness naming). Attach a skill (e.g. this one or a memory-usage example skill) for body pickup if the worker needs extra patterns or project facts. Review workers get read-only memory tools. No secret leakage — memory ops are server-side under the delegation's workspace scope.
...
7. Review the result as the parent when useful.
   ...
   - Do not grant workers access to linked Git metadata or rely on them to commit. The trusted runner snapshots checkout changes after post-execute validation, records the authoritative result branch/commit, and removes the temporary checkout only after terminal persistence. Verify those persisted fields before merging.
